...
## Docker y ejecución en contenedor

Incluí un `Dockerfile`, `docker-compose.yml` y un `Makefile` con objetivos útiles para construir y ejecutar la API y los scripts.

Construir la imagen y levantar la API (local):
...
Levantar un contenedor interactivo para ejecutar scripts / experimentar:

```bash
make build
docker run --rm -it -v ${PWD}:/app dataiatry-api:latest /bin/bash
```

Comandos rápidos con `Makefile` (requieren `make` y que el `venv` esté creado):

```bash
make venv       # crea .venv
make install    # instala dependencias en .venv
make test       # ejecuta pytest localmente
make train-xgb  # reentrena XGBoost final usando script
make plot       # genera la figura comparativa
```
