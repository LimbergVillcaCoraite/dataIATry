"""Genera SHAP summary para `models/model_xgb_hpo.joblib`.
Guarda `reports/figs/shap_xgb_summary.png` y `reports/shap_xgb_top_features.json`.
"""
...
def main():
    ...
    model_path = ROOT / 'models' / 'model_xgb_hpo.joblib'
    if not model_path.exists():
        raise FileNotFoundError('No se encontró model_xgb_hpo.joblib en models/')

    print('Cargando modelo XGBoost desde', model_path)
    model = joblib.load(model_path)

    # Alinear X si el modelo tiene feature_name_
    try:
        ...
        print('Advertencia al alinear features:', e)

    try:
        import shap

        print('Usando SHAP TreeExplainer para XGBoost (muestra para velocidad)')
        ...
        with open(ROOT / 'reports' / 'shap_xgb_top_features.json', 'w') as fh:
            json.dump({'model': str(model_path), 'top_features': top}, fh, indent=2)

        print('SHAP XGBoost guardado en', out)
        return
    ...
    try:
        ...
        plt.title('Permutation importance (mean) - XGBoost')
        plt.tight_layout()
        out = FIGS / 'perm_importance_xgb.png'
        plt.savefig(out, dpi=150)
        plt.close()
        ...
        print('Permutation importance XGBoost guardado en', out)
        return

    except Exception as e:
        raise RuntimeError('No se pudo generar SHAP ni permutation importance para XGBoost: ' + str(e))
