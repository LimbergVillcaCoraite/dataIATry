...
def get_feature_columns_example(df: pd.DataFrame) -> list:
    ...
    # prepare_X_y espera la columna objetivo; si no existe, añadimos una columna
    # temporal para evitar errores al generar las columnas numéricas de ejemplo.
    if 'total_alquileres' not in df2.columns:
        df2['total_alquileres'] = 0
        added_target = True
    else:
        added_target = False
    ...
    # limpiar columna temporal si la añadimos
    if added_target:
        df2.drop(columns=['total_alquileres'], inplace=True, errors='ignore')
